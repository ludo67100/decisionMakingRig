...
prob_to_give_reward_in_correct_trials = sph.PROB_REWARD_SIDE
prob_to_give_reward_in_error_trials = sph.PROB_NON_REWARD_SITE
...
for i in range(sph.NTRIALS):  # Main loop
    ...
    sma.add_state(
        ...
        state_timer=0, 
        output_actions=[("Serial1", bonsai_freeze_stim)],
        state_change_conditions={"Tup": "give_reward_anyway"},
    )
    
    sma.add_state(
        state_name="give_reward_anyway", 
        state_timer=tph.reward_valve_time,
        output_actions=[("Valve1", 255),("BNC2", 1)],
