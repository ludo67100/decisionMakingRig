...
#Determine trial type based on probability to give reward

prob_to_give_reward_in_correct_trials = 0 #sph.PROB_REWARD_SIDE

prob_to_give_reward_in_error_trials = 100 #sph.PROB_NON_REWARD_SITE
...
for i in range(sph.NTRIALS):  # Main loop
    ...
    print('Movement Left')
    print(tph.movement_left)
    print('Movement Right')
    print(tph.movement_right)
    print('Event_Error')
    print(tph.event_error)
    print('event reward')
    print(tph.event_reward)
    print(tph.threshold_events_dict)



    #Random choice based on probability reward
    pick = random.choice([x+1 for x in range(100)])

    #Determine type of correct and error trials 
    ...
    #TODO understand why blocks are updating tph.event_error and tph.event_reward afer 1st block. 
    ...
    sma.add_state(
        state_name="stim_on",
        state_timer=0.1,
        output_actions=[("Serial1", bonsai_show_stim),("BNC2", 1)],
        state_change_conditions={
    ...
    sma.add_state(
        ...
        state_timer=0.1,
        output_actions=[tph.out_tone,("BNC1", 1)],
        state_change_conditions={
    ...
    sma.add_state(
        ...
        state_change_conditions={
            "Tup": "no_go",
            tph.event_error: errorTrialType,
            tph.event_reward: correctTrialType,
        },
    )
    ...
    sma.add_state(
        state_name="error",
        state_timer=tph.iti_error,
        output_actions=[tph.out_noise,("BNC2", 1)],
    ...
    sma.add_state(
        ...
        output_actions=[("Valve1", 255),("BNC2", 1)],
    ...
    sma.add_state(
        state_name="freeze_reward_omit", 
        state_timer=0, 
        output_actions=[("Serial1", bonsai_freeze_stim)], 
        state_change_conditions={"Tup": "correct2"},
    )
    ...
    sma.add_state(
        state_name="freeze_error_give_reward", 
        state_timer=0, 
        output_actions=[("Serial1", bonsai_freeze_stim)],
        state_change_conditions={"Tup": "error_rewarded"},

    )

    sma.add_state(
        state_name="error_rewarded", 
        state_timer=tph.reward_valve_time,
        output_actions=[("Valve1", 255),("BNC2", 1)],
        state_change_conditions={"Tup": "error_no_sound"},

    )

    sma.add_state(

        state_name="error_no_sound", 
        state_timer=tph.iti_error, 
        output_actions=[], 
        state_change_conditions={"Tup": "hide_stim"}, 
    )
    ...
    print("Current trial info: ", bpod.session.current_trial.export())
