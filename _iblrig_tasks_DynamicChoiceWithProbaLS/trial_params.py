...
class TrialParamHandler(object):
    ...
    def __init__(self, sph):
        ...
        #Get reward probabilities - UPDATE LS 10/25/2022
        self.PROB_REWARD_SIDE = sph.PROB_REWARD_SIDE
        self.PROB_NON_REWARD_SITE = sph.PROB_NON_REWARD_SITE
        #self.reward_probabilities = self.randomize_reward(self.PROB_REWARD_SIDE,self.PROB_NON_REWARD_SITE)


        # Reward amount
        self.reward_amount = sph.REWARD_AMOUNT

        self.initial_reward_amount = sph.REWARD_AMOUNT #To keep the global variable
        ...
        #Dymanic choice vaalues LS MAY 2022

        self.TRIALS_IN_DYNAMIC_BLOC = sph.TRIALS_IN_DYNAMIC_BLOC
        self.EXPECTED_SUCCESS_RATE = sph.EXPECTED_SUCCESS_RATE
        self.DYNAMIC_CUTOFF = sph.DYNAMIC_CUTOFF


        #Initiate rolling buffer for dynamic reversal LS
        self.dynamicBuffer = []

	
        # RE event names
        self.listOfTrialDirections = sph.reward_magnitude()
	#Choose between left (1_1) or right (1_2) to assign reward side, based on list in sph 
        print('Trial #{}'.format(self.trial_num))
        print('Reward Magnitude = {}'.format(self.listOfTrialDirections[self.trial_num]))
        if self.listOfTrialDirections[self.trial_num] == -1:
            self.event_error = "RotaryEncoder1_2" #self.threshold_events_dict[self.position]
            self.event_reward = "RotaryEncoder1_1" #self.threshold_events_dict[-self.position]
            print('Error is set on {}'.format(self.event_error))
            print('Reward is set on {}'.format(self.event_reward))

        else:
            self.event_error = "RotaryEncoder1_1" #self.threshold_events_dict[self.position]
            self.event_reward = "RotaryEncoder1_2" #self.threshold_events_dict[-self.position]
            print('Error is set on {}'.format(self.event_error))
            print('Reward is set on {}'.format(self.event_reward))
...
##########################################"""
    ...
    def next_trial(self):
        ...
        if self.trial_num == 0:
            ...
            print('Current trial #{}'.format(self.trial_num))
            self.trial_num += 1
            self.block_num += 1
        ...
        #Update dynamic state LS
        self.dynamicChoiceUpdate()

        print('Reward Magnitude = {}'.format(self.listOfTrialDirections[self.trial_num]))  
        if self.listOfTrialDirections[self.trial_num] == -1:
            self.event_error = "RotaryEncoder1_2" 
            self.event_reward = "RotaryEncoder1_1" 
            print('Error is set on {}'.format(self.event_error))
            print('Reward is set on {}'.format(self.event_reward))

        else:
            self.event_error = "RotaryEncoder1_1" 
            self.event_reward = "RotaryEncoder1_2" 
            print('Error is set on {}'.format(self.event_error))
            print('Reward is set on {}'.format(self.event_reward))
    ...
    def dynamicChoiceUpdate(self):

        '''
        Updates the side of the reward depending on mouse's perf
        '''

        #self.trial_correct_buffer is a list of booleans containing the results of the trials 
        #IMPLEMENT DYNAMIC REVERSAL HERE
        successiveTrialsToConsider = self.TRIALS_IN_DYNAMIC_BLOC
        successRate = self.EXPECTED_SUCCESS_RATE

        #Initiate : the mouse needs to perform a couple of trials first 
        if len(self.dynamicBuffer) < successiveTrialsToConsider:
            print('Mouse did not perform enough trials {} out of {}: building up'.format(len(self.dynamicBuffer),successiveTrialsToConsider))

            print('Trial outcome in the block: {}'.format(self.dynamicBuffer))


        elif len(self.dynamicBuffer) == self.DYNAMIC_CUTOFF:
            print('Mouse reached cutoff : reverse reward')
            self.mouseReachedCutoff()

            self.dynamicBuffer = []

        #Once mouse has done enough trials, activate dynamic mode 
        else: 
            print('Mouse performed enough trials:dynamic mode')
            #self.rollingBuffer = self.trial_correct_buffer[-successiveTrialsToConsider:]
            self.rollingBuffer =self.dynamicBuffer[-successiveTrialsToConsider:]
            print('Last {} trials: {}'.format(successiveTrialsToConsider,self.rollingBuffer))
            correctTrials = np.count_nonzero(self.rollingBuffer)
            failedTrials = len(self.rollingBuffer)-correctTrials

            factor = 100./float(successRate)
            treshold = successiveTrialsToConsider/factor

            if correctTrials >= treshold:
                print('{} correct trials (treshold={})'.format(correctTrials,treshold))
                print('Mouse reached gap:reverse reward')
                self.reverse_magnitude()
                #Reset buffer
                self.dynamicBuffer = []

            else:
                print('{} correct trials (treshold={})'.format(correctTrials,treshold))
                print('Building up dynamic choice')


    def reverse_magnitude(self):
        '''

        #Inverse the magnitude list if mouse reached perf treshold 

        defined in dynamicChoiceUpdate()

        '''
        self.listOfTrialDirections = [-1*x for x in self.listOfTrialDirections]



    def mouseReachedCutoff(self):

        #Contrains next trial to be a sucess what ever the mouse decides
        #self.event_reward = ("RotaryEncoder1_2" ,"RotaryEncoder1_1")

        self.reverse_magnitude()
        print('Cutoff is reached: next trial will be rewarded anyway')
    ...
    def trial_completed(self, behavior_data):
        ...
        error_rewarded = ~np.isnan(self.behavior_data["States timestamps"]["error_rewarded"][0][0])
        correct_omit = ~np.isnan(self.behavior_data["States timestamps"]["freeze_reward_omit"][0][0])


        self.deliver_reward = 0

        print('self.deliver_reward', self.deliver_reward)


        print('correct', correct)
        print('error', error)
        print('error_rewarded', error_rewarded)
        print('correct_ommited', correct_omit)

        assert correct or error or no_go or error_rewarded or correct_omit
        ...
        elif (correct_omit and self.position < 0) or (error and self.position > 0):
            self.response_side_buffer.append(1)
        ...
        elif (correct and self.position > 0) or (error_rewarded and self.position < 0):
            self.response_side_buffer.append(-1)
        ...
        if correct:
            ...
            self.dynamicBuffer.append(self.trial_correct)
            self.ntrials_correct += self.trial_correct

            self.deliver_reward += 1

        elif correct_omit:
            self.trial_correct = bool(correct_omit)
            self.trial_correct_buffer.append(True)
            self.dynamicBuffer.append(True)
            self.ntrials_correct += self.trial_correct
        ...
        #Add LS May 2022 : update dynamicBuffer (now in if elif loop above)
        # Increment the trial correct counter (now in elif if loop above)
...
if __name__ == "__main__":
    ...
    correct_trial = {
        ...
        "States timestamps": {
            ...
            "no_go": [[np.nan, np.nan]],
            "error": [[np.nan, np.nan]],
            "correct_omit": [[np.nan, np.nan]],
            "error_rewarded": [[np.nan, np.nan]],
    ...
    correct_omit_trial = correct_trial
    correct_omit_trial['correct_omit'] = [[49.7357, 50.5787]]
    correct_omit_trial['correct'] = [[np.nan, np.nan]]
    correct_omit_trial['reward'] = [[np.nan, np.nan]]

    error_rewarded_trial = error_trial
    error_rewarded_trial["reward"] = [[49.5787, 49.7357]]
    error_rewarded_trial["error"] = [[np.nan, np.nan]]
    error_rewarded_trial["error_rewarded"] = [[13.4859, 15.4859]]
